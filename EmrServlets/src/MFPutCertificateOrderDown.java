...
public class MFPutCertificateOrderDown implements MFPut {
	private static MFPutCertificateOrderDown mInstance=null;
	private MFPutCertificateOrderDown(){
		
	}
	
	public static MFPutCertificateOrderDown getInstance(){
		if(mInstance==null){
			mInstance = new MFPutCertificateOrderDown();
		}
		return mInstance;
	}
		...
		try{
			...
			if(interfaceTableYn){
				sql =
					...
					" order by convert(numeric,disp_order)";
			}else{
				sql =
					...
					" order by convert(numeric,fld1cd)";
			}
			para.clear();
			...
			if(rs.getRecordCount()>0){
				dnCcfId = rs.getString(0, "mst3cd");
				dnOrderIdx = rs.getInt(0, "fld1cd");
			}
			if("".equalsIgnoreCase(dnCcfId)){
				return ExceptionHelper.toJSONString(-1, "맨 아래에 있습니다. 처리할 수 없습니다.");
			}
