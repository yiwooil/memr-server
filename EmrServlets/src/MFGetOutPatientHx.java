...
public class MFGetOutPatientHx implements MFGet {
	private static MFGetOutPatientHx mInstance=null;
	private MFGetOutPatientHx(){
		
	}
	private static HashMap<String, String> sqlMap=null; // sql문장을 메모리에 한번만 올려놓고 다음부터는 계속 사용한다.
	
	public static MFGetOutPatientHx getInstance(){
		if(mInstance==null){
			mInstance = new MFGetOutPatientHx();
	...
	private String getSqlInterOracleJaincom(){
		...
		if(sqlMap.containsKey(key)==false){
			String sql = 
					...
					"     , '' as age" +
					"     , '외래' as ward" +
					"     , (select a07.doctor_name from doctor_mast a07 where a07.doctor_id=s21.doctor_id and rownum<2) as pdrnm" +
					"     , s21.insu_class as qfycd" +
					"     , (select a88.insu_class_name from insu_class_mast a88 where a88.insu_class=s21.insu_class) as qfycdnm" +
					"     , (select s06.dise_name" +
					"          from out_pat_dise_hist s06" +
